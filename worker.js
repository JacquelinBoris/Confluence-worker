...
async function callInteractionOnce({ apiKey, systemInstruction, input, previousInteractionId, tools, schema }) {
  ...
  try {
    ...
    if (err.name === "AbortError") {
      throw new Error(`Timeout : l'agent n'a pas répondu en ${AGENT_TIMEOUT_MS / 1000}s.`);
    }
    throw err;
  } finally {
...
async function runPipeline(apiKey, imageBase64, mimeType, videoBase64, videoMimeType) {
  ...
  const visionRes = await callInteraction({
    apiKey, systemInstruction: PROMPTS.vision, input: visionInput, schema: SCHEMAS.vision,
  });
