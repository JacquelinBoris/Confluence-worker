...
async function callGroq({ apiKey, model, messages, useJsonMode }) {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      const isJsonGenFail = /\(400\)/.test(err.message) && /json_validate_failed|Failed to validate JSON/i.test(err.message);
      const retriable = is429 || /\(50[0-9]\)/.test(err.message) || /\(413\)/.test(err.message) || isJsonGenFail;
...
async function callGroqOnce({ apiKey, model, messages, useJsonMode }) {
  const body = { model, messages, temperature: 0.3, max_tokens: 2048 };
